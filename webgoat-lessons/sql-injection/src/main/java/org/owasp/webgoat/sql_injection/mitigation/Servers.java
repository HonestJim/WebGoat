...
public class Servers {
    ...
    public List<Server> sort(@RequestParam String column) throws Exception {
        ...
        try (Connection connection = dataSource.getConnection();
             // Only allow safe, known column names (example):
             String safeColumn = switch (column) {
                 case "id", "hostname", "ip", "mac", "status", "description" -> column;
                 default -> throw new IllegalArgumentException("Invalid column name");
             };
             PreparedStatement preparedStatement = connection.prepareStatement("select id, hostname, ip, mac, status, description from servers  where status <> 'out of order' order by " + safeColumn)) {
